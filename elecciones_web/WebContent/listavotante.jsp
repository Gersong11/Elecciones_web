            ...
            <main class="w-full flex-grow p-6">
                ...
                <div class="w-full mt-6">
                    <p class="text-xl pb-3 flex items-center">
                        <i class="fas fa-list mr-3"></i> Votantes
                    </p>
                        ...
                        <table class="min-w-full bg-white">
                            <thead class="bg-gray-800 text-white">
                                <tr>
                                    ...
                                    <th class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm">Elección</th>
                                    <th class="text-left py-3 px-4 uppercase font-semibold text-sm">Acciones</th>
                                </tr>
                            </thead>
                            <tbody class="text-gray-700">
                            	<c:forEach var="c" items="${listaVotante}">
                                <tr>
                                    <td class="w-1/3 text-left py-3 px-4"><c:out value="${c.nombre}" /></td>
                                    <td class="text-left py-3 px-4"><c:out value="${c.email}" /></td>
                                    <td class="text-left py-3 px-4"><c:out value="${c.documento}" /></td>
                                    <td class="text-left py-3 px-4"><c:out value="${c.tipodocumentoBean.descripcion}" /></td>
                                    <td class="w-1/3 text-left py-3 px-4"><c:out value="${c.eleccionBean.nombre}" /></td>
                                    <td class="text-left py-3 px-4">
                                        <a href="ActualizarVotanteController?id=${c.id}"><i class="far fa-edit"></i>
                                        <a href="EliminarVotanteController?id=${c.id}"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                                </c:forEach>
                            </tbody>
