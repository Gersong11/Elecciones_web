        ...
        <div class="w-full h-screen overflow-x-hidden border-t flex flex-col">
            <main class="w-full flex-grow p-6">
                <h1 class="w-full text-3xl text-black pb-6 text-center">
                	<c:if test = "${candidato.id == null}">Registrar candidato</c:if>
                	<c:if test = "${candidato.id != null}">Actualizar candidato</c:if>
                </h1>
                ...
                <div class="flex flex-wrap">
                    <div class="w-full lg:w-1/2 my-6 pr-0 lg:pr-2 mx-auto">
                        ...
                        <div class="leading-loose">
                        	<c:if test = "${candidato.id == null}">
                               <form class="p-10 bg-white rounded shadow-xl"  action="RegistrarCandidatoController" method="post">
                             </c:if>
                             <c:if test = "${candidato.id != null}">
                               <form class="p-10 bg-white rounded shadow-xl"  action="ActualizarCandidatoController" method="post">
                             </c:if>
                                <div class="">
                                    <label class="block text-sm text-gray-600" for="name">Nombre</label>
                                    <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id=""nombre"" name="nombre" type="text" required="" placeholder="Gerson" aria-label="Name"
                                    <c:if test = "${candidato.id != null}">
                                         value="${candidato.nombre}"
                                    </c:if>>
                                </div>
                                <div class="">
                                    <label class="block text-sm text-gray-600" for="name">Apellido</label>
                                    <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="apellido" name="apellido" type="text" required="" placeholder="Fernando" aria-label="Name"
                                    <c:if test = "${candidato.id != null}">
                                ...
                                <div class="mt-2">
                                    ...
                                    <c:if test = "${candidato.id != null}">
                                         value="${candidato.documento}"
                                    </c:if>>
                                </div>
                                <div class="mt-2">
                                    <label class="block text-sm text-gray-600" for="email">Número</label>
                                    <input class="w-full px-5  py-4 text-gray-700 bg-gray-200 rounded" id="numero" name="numero" type="text" required="" placeholder="1" aria-label="Email"
                                    <c:if test = "${candidato.id != null}">
                                ...
                                <div class="mt-6">
                                    <button class="px-4 py-1 text-white font-light tracking-wider bg-gray-900 rounded" type="submit">
                                    	<c:if test = "${candidato.id == null}">Registrar</c:if>
                               			<c:if test = "${candidato.id != null}">Actualizar</c:if>
                               		</button>
                                </div>
                            </form>
                        </div>
