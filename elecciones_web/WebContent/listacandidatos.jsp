            ...
            <main class="w-full flex-grow p-6">
                ...
                <div class="w-full mt-6">
                    <p class="text-xl pb-3 flex items-center">
                        <i class="fas fa-list mr-3"></i> Candidatos
                    </p>
                    <div class="bg-white overflow-auto">
                        <table class="min-w-full bg-white">
                            <thead class="bg-gray-800 text-white">
                                <tr>
                                    <th class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm">Documento</th>
                                    ...
                                    <th class="text-left py-3 px-4 uppercase font-semibold text-sm">eleccion</th>
                                    <th class="text-left py-3 px-4 uppercase font-semibold text-sm">Acciones</th>
                                </tr>
                            </thead>
                            <tbody class="text-gray-700">
                            	<c:forEach var="c" items="${listaCandidato}">
                                <tr>
                                    <td class="w-1/3 text-left py-3 px-4"><c:out value="${c.documento}" /></td>
                                    <td class="w-1/3 text-left py-3 px-4"><c:out value="${c.nombre}" /></td>
                                    <td class="text-left py-3 px-4"><c:out value="${c.apellido}" /></td>
                                    <td class="text-left py-3 px-4"><c:out value="${c.numero}" /></td>
                                    <td class="text-left py-3 px-4"><c:out value="${c.eleccionBean.nombre}" /></td>
                                    <td class="text-left py-3 px-4">
                                        <a href="ActualizarCandidatoController?id=${c.id}"><i class="far fa-edit"></i></a>
										<a href="EliminarCandidatoController?id=${c.id}"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                                </c:forEach>
                            </tbody>
